...
void Live::init(Displayer *displayer, Config *config, StrategyCollection *collection, PoolWorker *poolWorker, Database *database)
{
    ...
    for (MarketConfig *mc : config->getConfiguredMarkets()) {
        ...
        // strategy->prepareMarket(); ...
    }

    // create and start a connector in this thread
...
{
    return static_cast<o3d::Double>(o3d::System::getTime()) / o3d::System::getTimeFrequency();
}
...
o3d::Int32 Live::run(void *)
{
    ...
    while (m_running) {
        if (m_connector->traderProxy()) {
//            if (m_connector->traderProxy()->hasMessages()) {
