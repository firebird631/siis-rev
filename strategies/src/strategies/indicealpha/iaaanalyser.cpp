...
IaAAnalyser::IaAAnalyser(
    ...
    IaAnalyser(strategy, timeframe, subTimeframe, depth, history, priceMethod),
    m_atr("atr", timeframe),
    m_sma("sma", timeframe),
    m_ema("ema", timeframe),
    m_rsi("rsi", timeframe),
...
void IaAAnalyser::compute(o3d::Double timestamp, o3d::Double lastTimestamp)
{
    m_lastSignal.reset();
    ...
    if (m_td9.c().c >= 1 && m_td9.c().c <= 6 && m_td9.c().d < 0 && lvl1Signal > 0 /* && volumeSignal > 0*/) {
        ...
        // log("content", o3d::String("Entry long tdst={0} stop={1}").arg(m_td9.c().tdst).arg(m_lastSignal.sl()));
    }
    ...
    // short entry on buy-setup
        ...
        // log("content", o3d::String("Entry short tdst={0} stop={1}").arg(m_td9.c().tdst).arg(m_lastSignal.sl()));
    }
    ...
    // aggressive long entry
        ...
        // log("content", o3d::String("Aggressive long entry close={0} stop={1}").arg(m_lastSignal.p()).arg(m_lastSignal.sl()));
    }
    ...
    // aggressive short entry
        ...
        // log("content", o3d::String("Aggressive short entry close={0} stop={1}").arg(m_lastSignal.p()).arg(m_lastSignal.sl()));
    }
